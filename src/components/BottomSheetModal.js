import { View, Text, StyleSheet, Animated, PanResponder, Modal } from 'react-native'
...
const MAX_UPWARD_TRANSLATE_Y = BOTTOM_SHEET_MIN_HEIGHT - BOTTOM_SHEET_MAX_HEIGHT; // 음수
...
export default function BottomSheetModal({children}) {
  ...
  const springAnimation = (dirction) => {
    ...
    lastGestureDy.current = dirction === 'down' ? MAX_DOWNWARD_TRANSLATE_Y : MAX_UPWARD_TRANSLATE_Y;
  ...
  const bottomSheetAnimation = {
    transform: [{translateY: animatedValue.interpolate({
      inputRange: [MAX_UPWARD_TRANSLATE_Y + BOTTOM_SHEET_MIN_HEIGHT, MAX_DOWNWARD_TRANSLATE_Y],
      outputRange: [MAX_UPWARD_TRANSLATE_Y + BOTTOM_SHEET_MIN_HEIGHT, MAX_DOWNWARD_TRANSLATE_Y],
      extrapolate: 'clamp',
    })}],
  };
