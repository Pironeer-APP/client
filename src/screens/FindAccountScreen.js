import { View, Text, KeyboardAvoidingView, StyleSheet, Alert, Modal } from 'react-native'
...
export default function FindAccountScreen({ navigation }) {
  ...
  const onPressFindInfo = async () => {
    ...
    setLoading(false);
    if (res.result) {
      Alert.alert('새로운 비밀번호가 등록된 이메일로 전송되었습니다', '로그인 후 비밀번호를 변경해 주세요', [
        {
          text: '확인',
  ...
  return (
    <StyledContainer>
      ...
      >
        <View style={{ flex: 1 }}>
          <Gap />
          <StyledText fontSize={25} content={`전화번호(아이디)를 입력해 주세요`} />
          <Gap height={10} />
          <SettingInput
        ...
        >
          <View style={styles.circles}>
            <Progress.CircleSnail style={styles.progress} color={COLORS.green} />
          </View>
        </Modal>
